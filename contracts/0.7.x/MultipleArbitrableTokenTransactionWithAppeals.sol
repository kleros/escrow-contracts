 ...
 */
contract MultipleArbitrableTokenTransactionWithAppeals is
    IArbitrable,
    IEvidence
{
    ...
    function fundAppeal(
        ...
        if (remainingETH > 0) msg.sender.send(remainingETH); // Deliberate use of send in order to not block the contract in case of reverting fallback.
